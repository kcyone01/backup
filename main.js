...
const sndLevelup  = new THREE.Audio(listener);
...
// Level-up reuses shoot sound pitched up — no extra file needed
...
texLoader.load("./assets/images/cactus.webp", tex => {
  ...
  cactusMaterial = new THREE.SpriteMaterial({ map: tex, transparent: true, depthTest: true, depthWrite: false });
});
texLoader.load("./assets/images/background.png", tex => (scene.background = tex));

// ─── High Score Storage ──────────────────────────────────────────────────────
// Keys per difficulty so each board is separate.
...
function setHighScore(diff, val) {
  localStorage.setItem(HS_KEYS[diff], val);
}

// ─── Difficulty Presets ───────────────────────────────────────────────────────
// Each preset: { speed, interval(ms), strafeSpeed, label }
const DIFFICULTIES = {
...
// ─── Anti-cheat state object ─────────────────────────────────────────────────
const XORKEY = 0xCAFE;
const state = {
  _score: 0, _shadowScore: 0,
  _health: 100, _shadowHealth: 0,
...
function checkIntegrity() {
  if ((XORKEY ^ state._score) !== state._shadowScore || (XORKEY ^ state._health) !== state._shadowHealth) {
...
// Difficulty-scaling variables (reset each game)
...
let currentStreak   = 0;
let bestStreak      = 0;
let comboMultiplier = 1;   // 1x, 2x, 3x
let comboTimer      = 0;   // ms since last hit
const COMBO_WINDOW  = 3000; // ms to maintain combo
const COMBO_THRESHOLDS = [5, 10]; // hits to reach 2x, 3x
...
let rng = Math.random; // default to Math.random; overridden for daily
...
// Level indicator (injected like original)
...
function updateComboHud() {
  ...
  // Bar depletes over combo window
...
function triggerLevelUpFlash(lvl) {
  ...
  void elLevelFlash.offsetWidth; // reflow
...
function shoot() {
  ...
  if (hits.length > 0) {
    ...
    comboTimer = 0;
    if (currentStreak >= COMBO_THRESHOLDS[1])      comboMultiplier = 3;
    else if (currentStreak >= COMBO_THRESHOLDS[0]) comboMultiplier = 2;
    ...
    if (state.score >= scoreForLevel(level)) {
      ...
      playSound(sndShoot); // repurpose as level-up audio cue
...
function endGame(won) {
  ...
  if (won) {
    elVictOverlay.style.display = "flex";
  } else {
    elDeathOverlay.style.display = "flex";
  }
...
document.getElementById("daily-btn").onclick = () => {
  ...
  selectedDifficulty = "standard"; // daily always runs standard
  startGame();
};
